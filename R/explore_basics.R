...
rt_explore_plot_time_series_change <- function(dataset,
    ...
    if(is.null(color_variable) && is.null(facet_variable)) {
        ...
        ggplot_object <- change_gain_loss_by_group %>%
            ...
            geom_bar(aes(fill=!!sym_color_variable), stat='identity', position = position_dodge(width=0.5)) +
        ...
        ggplot_object <- change_gain_loss_by_group %>%
            ggplot(aes(x=period_label, y=!!symbol_y)) +
            geom_bar(aes(fill=!!sym_color_variable), stat='identity', position = position_dodge(width=1)) +
...
private_create_gain_loss_total_by_group <- function(dataset,
    ...
    if(is.null(color_variable)) {
        ...
        sym_color_variable <- sym('')

    } else {

        sym_color_variable <- sym(color_variable)
    }

    if(is.null(facet_variable)) {

        sym_facet_variable <- sym('')

    } else {

        sym_facet_variable <- sym(facet_variable)
    }
    ...
    if(is.null(aggregation_variable)) {
        ...
        change_gain_loss_by_group <- change_gain_loss_by_group %>%
            count(!!sym(date_variable), !!sym_color_variable, !!sym_facet_variable)

    } else {

        change_gain_loss_by_group <- change_gain_loss_by_group %>%
            group_by(!!sym(date_variable), !!sym_color_variable, !!sym_facet_variable) %>%
            summarise(n=aggregation_function(!!sym(aggregation_variable)))
    }
